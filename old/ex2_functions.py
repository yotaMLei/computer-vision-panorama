...
#     # print(max(warp_coords[2,:]), min(np.abs(warp_coords[2,:])))
    ...
    # print(max(x_ori), max(y_ori))
    # print(max(x_warp), max(y_warp))
    ...
    # if max(x_warp) > max(y_warp):
    #     max_cor = max(x_warp)
    # else : max_cor = max(y_warp)

    # Map the pixel RGB data to new location in another array
    # if max_cor > 10000 :
    #     # scale to minimize image
    #     scale = 100/max_cor
    #     # S = np.array([[scale, 0, 0], [0, scale, 0],[0, 0, 1]])
    #     # warp_coords_small = S@warp_coords
    #     x_warp, y_warp = x_warp*scale, y_warp*scale
    #     x_warp = x_warp.astype(int)
    #     y_warp = y_warp.astype(int)
    #     print(max(x_warp), max(y_warp))
...
#
#     # find inliners
#     inliners = mp_src[:, errors <= max_err]
#     inliners_err = errors[errors <= max_err]
#     fit_per = inliners.size / mp_src.size
#     dist_mse = np.mean(np.square(inliners_err))
#
#     # print(inliners.shape, inliners.size)
#
...
def compute_homography(match_p_src, match_p_dst, inliers_percent, max_err):
    ...
    # we want certainty of 99.99%
#     # print(f'num of iterations = {num_of_iteration}')
...
def get_mapped_corners(img, H):
    ...
    # print(bound_x_max, bound_x_min, bound_y_max, bound_y_min)
...
def panorama(img_src, img_dst, mp_src, mp_dst, inliers_percent, max_err):
    ...
    # print("rect width = ", src_rect_width)
    # print("rect height=", src_rect_height)
